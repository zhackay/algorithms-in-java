...
public class SearchRotatedArray {
    ...
    static int searchRotatedArray3(int[] a, int k, int lo, int hi) {
        if (lo > hi) return -1;
    ...
    static int searchRotatedArray2(int[] a, int k, int lo, int hi) {
        if (lo > hi) return -1;

        int mid = lo + (hi-lo)/2;
        if (a[mid] == k) return mid;

        if (a[mid] > a[lo] && a[lo] <= k && k < a[mid]) return searchRotatedArray(a, k, lo, mid-1);
        if (a[mid] < a[hi] && a[mid] < k && k <= a[hi]) return searchRotatedArray(a, k, mid+1, hi);
        if (a[mid] < a[lo]) return searchRotatedArray(a, k, lo, mid-1);
        if (a[mid] > a[hi]) return searchRotatedArray(a, k, mid+1, hi);
        return -1;
    }
